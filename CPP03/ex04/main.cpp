...
int main() {
	...
	SuperTrap	SuperClass("SUPER MEGATRON");
	
	SuperClass.vaulthunter_dot_exe(Ninja.getName());
	Rick.takeDamage(SuperClass.ninjaShoebox(Rick));
	cmdRangedAttack(SuperClass, Rick);
	Morty.takeDamage(SuperClass.rangedAttack(Morty.getName()));
	SuperClass.listСhar();
	...
	cmdMeleeAttack(SuperClass, Morty);
	cmdMeleeAttack(SuperClass, Morty);
	return 0;
}