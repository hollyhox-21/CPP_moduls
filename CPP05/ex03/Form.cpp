...
void Form::execute(const Bureaucrat &executor) {
	if (_signForm && (executor.getGrade() <= _gradeExecute)) {
		action(_target);
	}
	else if (!_signForm)
	{
		throw GradeTooLowException("Form is not sign");
	}
	else
		throw GradeTooLowException("Low grade for Execute");
}
...
std::ostream & operator<<(std::ostream & o, const Form &obj) {
	o	<< BRIGHT_RED
		...
		<< obj.getGradeExecute() << " - grade for execute"
		<< std::endl;
	return o;
}